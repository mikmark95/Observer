    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if mode == "emlid":
            ...
            for day, group in grouped:
                ...
                output_df['geometry'] = gpd.points_from_xy(group['Longitude'], group['Latitude'])
                geo_df = gpd.GeoDataFrame(output_df, geometry='geometry', crs='EPSG:4326')

                safe_day = re.sub(r'[\\/:*?"<>|]', '_', str(day))
                output_file = os.path.join(output_dir, f"dati_{safe_day}.{export_format}")

                if export_format == "csv":
                    geo_df.drop(columns='geometry').to_csv(output_file, index=False)
                elif export_format == "shp":
                    geo_df.to_file(output_file, driver="ESRI Shapefile")

                print(f"File esportato: {output_file}")
            ...
            for (squadra, day), group in grouped:
                ...
                output_df['geometry'] = gpd.points_from_xy(output_df['Long'], output_df['Lat'])
                geo_df = gpd.GeoDataFrame(output_df, geometry='geometry', crs='EPSG:4326')
                ...
                output_file = os.path.join(squadra_dir, f"dati_{safe_day}.{export_format}")
            ...
            for (squadra, day), group in grouped:
                ...
                output_df['geometry'] = gpd.points_from_xy(output_df['X'], output_df['Y'])
                geo_df = gpd.GeoDataFrame(output_df, geometry='geometry', crs='EPSG:4326')
